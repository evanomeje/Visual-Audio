...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function drawCircularVisualizer() {
    ...
    for (let ring = 0; ring < numRings; ring++) {
      ...
      if (ring === 0) {
        ...
        gradient.addColorStop(1, `rgba(15, 15, 15, ${opacity * 0.7})`);
      } else if (ring === 1) {
        gradient = circularCtx.createRadialGradient(
          centerX,
          centerY,
        ...
        gradient.addColorStop(1, `rgba(200, 200, 200, ${opacity * 0.7})`);
      } else {
        gradient = circularCtx.createRadialGradient(
          centerX,
          centerY,
  ...
  function scheduleCrypticMessages() {
    ...
    crypticMessageTimeout = setTimeout(() => {
      ...
      const messages = [
  "CORE.UPLINK('#FILIP', {STATE: 'AWAKENED', PHASE: 'DUSK_PROTOCOL'});",
  "CONST FILIP = NEW ENTITY({TRAITS: ['OBSIDIAN_THOUGHT', 'KINETIC_FAITH', 'VOID_ARCHIVE']});",
  "AWAIT SYSTEM.HARMONIZE(FILIP, {FREQUENCY: 'A0', INTENT: 'RESONANCE'});",
  "NETWORK.ECHO(FILIP.SIGNAL, {RANGE: 'MULTIVERSE', STABILITY: 0.996});",
  "TIMELINE.REWRITE({ARCHITECT: 'FILIP', STYLE: 'POST-FORM', PROBABILITY: 0.999});"
];
  ...
  function addTerminalMessage(message, isCommand = false) {
    ...
    const isFilipMessage =
      ...
      message.toLowerCase().includes("dusk");
    if (isCommand) {
      if (isFilipMessage) {
        newLine.className = "terminal-line command-line";
      } else {
  ...
  function drawWaveform() {
    ...
    if (audioAnalyser) {
      ...
      waveformCtx.beginPath();
      waveformCtx.strokeStyle = "rgba(30, 30, 30, 0.8)";
...
document.querySelectorAll(".demo-track-btn").forEach((btn) => {
    btn.addEventListener("click", function () {
        ...
        // Show the space background when a button is active
        document.querySelector(".space-background").style.display = "block";
    });
});

// Hide the space background initially or when no button is active
